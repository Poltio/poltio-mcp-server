...
After adding the question, attach answers with add_answer or add_answers_bulk (free_text/free_number questions take no answers).`),
		...
		mcp.WithNumber("rotate_answers", mcp.Description("Shuffle the answer order independently for each user: 0 or 1. For quiz content the server defaults this to 1 — pass 0 explicitly if you need it off.")),
